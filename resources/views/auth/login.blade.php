...
<div class="pp-login">
    ...
    <div class="pp-login__slot">
        <button type="button" onclick="handleKakaoLogin()" class="pp-login__btn pp-login__btn--kakao">
            @if($lastLogin === 'kakao')<span class="pp-login__last">최근</span>@endif
    ...
    <div class="pp-login__slot">
        <button type="button" onclick="handleNaverLogin()" class="pp-login__btn pp-login__btn--naver">
            @if($lastLogin === 'naver')<span class="pp-login__last">최근</span>@endif
    ...
    <div class="pp-login__slot">
        <button type="button" onclick="handleGoogleLogin()" class="pp-login__btn pp-login__btn--google">
            @if($lastLogin === 'google')<span class="pp-login__last">최근</span>@endif
    ...
    <div class="pp-login__slot pp-login__slot--apple" id="ppAppleSlot" hidden>
        <button type="button" onclick="handleAppleLogin()" class="pp-login__btn pp-login__btn--apple">
            @if($lastLogin === 'apple')<span class="pp-login__last">최근</span>@endif
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.48-3.24 0-1.44.62-2.2.44-3.06-.4C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
            Apple로 시작하기
        </button>
    </div>

    <div class="pp-login__consent">계속하면 <a href="{{ route('terms', ['tab' => 'terms']) }}">이용약관</a> · <a href="{{ route('terms', ['tab' => 'privacy']) }}">개인정보처리방침</a> · <a href="{{ route('terms', ['tab' => 'location']) }}">위치기반서비스 이용약관</a>에 동의하게 됩니다.</div>

    <a href="/" class="pp-login__browse">로그인 없이 둘러보기</a>
    <div class="pp-login__browse-sub">최대 5개까지 임시 저장할 수 있어요</div>
